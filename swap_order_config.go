...
type SwapOrderItem struct {
	...
	OrderPrice  string `json:"order_price"`  // 下单价格
	TradePrice  string `json:"trade_price"`  // 成交价格
	OrderValue  string `json:"order_value"`  // 下单金额
	TradeValue  string `json:"trade_value"`  // 成交金额
	OrderVolume string `json:"order_volume"` // 下单数量
	TradeVolume string `json:"trade_volume"` // 成交数量
	Status      string `json:"status"`       // 订单状态 1-已下单 2-已成交 8-已撤单
